...
export function MapRenderer(props: MapRendererProps) {
  ...
  const rasterFadeDuration = useMemo(() => {
    if (!radar.enabled) return 0;
    if (useLocalImage) return 0;
    return liveZoom >= RADAR_CRISP_MIN_ZOOM ? 120 : 180;
  }, [liveZoom, radar.enabled, useLocalImage]);
