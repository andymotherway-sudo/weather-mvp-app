...
- `NEXRAD Level III cycle` publishes the initial Phase 1 site bundle (`IWA`, `MPX`, `DLH`) and product bundle (`N0B`, `N0S`, `EET`) in one bounded run. Its cron is gated by `LEVEL3_SCHEDULE_ENABLED`; scheduled site scope is controlled by `LEVEL3_SCHEDULE_SITES` and defaults safely to `IWA` if that variable is missing.
